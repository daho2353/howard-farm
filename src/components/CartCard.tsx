...
const CartCard: React.FC<Props> = ({product, removeFromCart, decreaseCartQuantity, increaseCartQuantity}) => {
    ...
    return(
        ...
        <p className="product-quantity">Number in cart: <button onClick={() => decreaseCartQuantity(product)}> - </button> {product.quantityInCart} <button onClick={() => increaseCartQuantity(product)} disabled={isStockExceeded} className={isStockExceeded ? "disabled-button" : ""} > +</button> </p>
